...
public:
    PeerNode(string ip, const size_t port, string seedfile,
             const long interArrivalTime, const double nodeHashPower,string floodFile)
            : mIp(ip),
              mPort(port),
              mSeedFile(std::move(seedfile)),
              handler(ip, port, interArrivalTime, nodeHashPower, floodFile) {}
...
//            cerr << "PeerNode::handlePeerLiveliness: Could not write data to "
...
//                cout << "PeerNode::handlePeerLiveliness: No reply from "
//                     << peerIp << ":" << peerPort << " on " << mIp << ":"
//                     << mPort << endl;
                dead++;
            }
        }
...
void PeerNode::init() {
    ...
    (new thread([this]() {
        this->handler.startBroadcastTask(this->mNeighbors, this->mPeerListMutex);
    }))->detach();
    ...
    while (true) {
        ...
        while (i < nfds) {
            ...
            if (s.length() > 0) {
                ...
                auto output = handler.handle(events[i].data.fd, s,
                                             mNeighbors, mPeerListMutex);
...
void printUsage(string s) {
    cout << "Usage: " << s
         << " -ip=XXX.XXX.XXX.XXX -port=XXXX -seedfile=XXX "
            "-interArrivalTime=XX -nodeHashPower=XX -floodFile=XXX"
         << endl;
    exit(EXIT_FAILURE);
}
...
int main(int argc, char const *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (i == 6 and (pos = arg.find('=')) != string::npos) {
            if (arg.substr(1, pos - 1) == "floodFile") {
                floodFile = arg.substr(pos + 1, arg.length());
                cout << "floodFile: " << floodFile << endl;
            } else {
                printUsage(string(argv[0]));
            }
        } else {
    ...
    if (Formatter::validateIP(ip)) {
        PeerNode node(ip, port, seedfile, interArrivalTime, nodeHashPower,floodFile);
        node.init();
        node.start();
    } else {
